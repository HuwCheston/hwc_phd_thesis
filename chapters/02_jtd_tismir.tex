...
In this chapter, we describe several related datasets, discuss the curation of \GLS{JTD}, outline the data extraction pipeline we developed, and explore \GLS{JTD} by conducting several analyses. We envisage that \GLS{JTD} will be useful in a variety of \GLS{MIR} tasks, such as artist identification (Chapters \ref{chap:xai_rsi}, \ref{chap:rhythm_rsos}), symbolic music generation (Chapter \ref{chap:conditioned_gen}), and expressive performance modelling.

\section{Related Work}

\begin{sidewaystable}[ph!]
\centering
\caption[Comparison of existing datasets for jazz piano trios.]{Comparison of existing datasets for each instrument in the jazz piano trio. *Note that RWC-Jazz has not been made available free and open source, meaning that it is not possible to provide full detail here. See also Table \ref{tab:gen_finetuning_dataset}.}
\label{tab:jtd_existing_datasets}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{p{2.2cm} p{2.5cm} p{2.8cm} c c p{4cm} p{3.5cm}}
\toprule
...
\textbf{Piano} & \GLS{WJD} & Manual & 6 & 582 & 3,149 onsets & Beat, chord, section \\
 & \GLS{PiJAMA} & Automatic & 2,777 & 786,206 & 7,128,802 MIDI notes & N/A \\
 & RWC-Jazz & Manual & 5 & 1,672 & N/A* & Beat, section \\
 & \textbf{\GLS{JTD} (ours)} & Automatic & 1,294 & 159,668 & 866,116 onsets, 2,174,833 MIDI notes & Beat \\
\midrule
\textbf{Bass} & \GLS{WJD} & Automatic & 426 & 49,010 & 5,000 beat-wise pitches & Beat, chord, section \\
 ...
 & RWC-Jazz & Manual & 5 & 1,672 & N/A* & Beat, section \\
 & \textbf{\GLS{JTD} (ours)} & Automatic & 1,294 & 159,668 & 543,693 onsets & Beat \\
\midrule
...
FiloBass \citep{Riley2023} expands FiloSax with transcriptions of the bassist's performance on each backing track. A source separation model was first applied to isolate this from the audio mixture, and a commercial automatic transcription algorithm was used to generate MIDI from this signal. The resulting transcriptions were then checked manually and corrected when necessary. FiloBass represents an important effort to use recent developments in music source separation to facilitate downstream annotation, which we expand upon here by providing annotations for every member in an ensemble.

\subsection{PiJAMA}\label{sec:jtd_pijama_description}

\added[id=HC]{The} Piano Jazz with \added[id=HC]{Automatic} MIDI Annotations (\GLS{PiJAMA}) \added[id=HC]{dataset} consists of 200+ hours of solo jazz piano performances transcribed using a pitch-to-MIDI algorithm \citep{Edwards2023}. The methodology used by the \GLS{PiJAMA} authors has several similarities with our contribution: curated playlists of relevant music were developed by scraping discographic services, and audio was downloaded from YouTube. Although comprehensive, \GLS{PiJAMA} includes solo piano recordings only, while the authors acknowledge that it is far more common for jazz pianists to perform in a trio with bass and drums. 

In Chapter~\ref{chap:xai_rsi}, we combine a subset of \GLS{PiJAMA} with \GLS{JTD} to create a large dataset usable in jazz performer identification. In Chapter \ref{chap:conditioned_gen}, we use the entire \GLS{PiJAMA} dataset (alongside the entire \GLS{JTD}, and several other datasets) to create what we believe to be the largest dataset of MIDI transcriptions of solely jazz piano performances (approximately 300 hours of performances, see Table \ref{tab:gen_finetuning_dataset}). We then use this dataset to train a music generation model to produce music conditioned on particular styles of jazz.
...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/jtd/figure_1_alt.png}
...
We then cross-reference the Last.fm results against two major jazz textbooks, keeping only those artists that receive a mention in the discographies of either Ted Gioia's ``The History of Jazz'' \citeyearpar{Gioia2011} or Mark Levine's ``The Jazz Piano Book'' (\citeyear{Levine2011-1}a). The intention here is to capture artists regarded highly by jazz experts. We note, however, that the retrospective nature of these textbooks could exclude performers who have become active only in recent years (or, indeed, in the time since their publication). Future revisions of \GLS{JTD} may use subsequent editions of both textbooks. 
...
To obtain a consistent set of tracks that our automated pipeline can analyse reliably, we define inclusion criteria to compare against each recording before including it in \GLS{JTD}. These requirements are set to (1) ensure that all notes have clear onset times, and (2) maximise the acoustic similarity between the recordings in the dataset and the distribution of (mostly pop \& rock) material typically used to train audio source separation models. 

Our inclusion criteria are as follows. A track must have: (1) an approximate tempo between 100 and 300 quarter-note beats-per-minute (BPM)\deleted[id=HC]{, assessed by tapping along to the opening measures of the performance,} (2) a clear quarter note pulse, with a time signature of either three or four beats per measure (and with no changes in meter), (3) an identifiable piano solo, accompanied by bass and drums with no interruptions in the ensemble texture (i.e., ``solo breaks''), (4) an uninterrupted ``swing eighths'' rhythmic feel, and (5) a link to the recording on YouTube. \added[id=HC]{The first four parts of the inclusion criteria were tested by listening to the entire recording.  For example, the ``tap tempo'' functionality of a metronome was used to measure the approximate tempo of the opening measures of each performance.}

Additional criteria are specified for each instrument in the trio. During their solo, pianists must have played on acoustic instruments (rather than, e.g., synthesiser or Fender ``Rhodes'' piano), and without any external FX (e.g., reverb, distortion). Bassists must also have played on an unmanipulated acoustic instrument using their fingers, not a bow. Drummers must have used a traditional ``traps'' kit (snare and kick drums; multiple cymbals, including hi-hat, ride, and crash; tom-toms), without auxiliary percussion (e.g., shakers, maracas). This must also have been played using sticks, rather than wire brushes or cloth mallets. 

\subsubsection{Metadata Curation}\label{sec:jtd_metadata_curation}

For the recordings that met the inclusion criteria, we combine the metadata scraped from MusicBrainz with additional fields compiled manually, including timestamps for the beginning and ending of the piano solo, the position of individual instrument sources across the stereo spectrum, and the time signature (used later in Section \ref{sec:gen_tokenization}). YouTube URLs are scraped from the ListenBrainz service using the track identifiers obtained from MusicBrainz and are manually corrected in the case of any false positives. Audio is downloaded from the URLs, trimmed to the piano solo, and stored in a lossless format with a sample rate of 44.1 kHz.

We elect to include audio from piano solos only, as this is typically the only section in a performance where every musician would be expected to improvise. For instance, in the ``head'' (melodic statements that occur at the beginning and ending of ``straight ahead'' jazz performances), the material is pre-composed; while, in bass or drum solos, the accompanying musicians may choose not to play \citep{Monson1996}.

\begin{figure}[!ht]
  \centering
  \includesvg[width=0.75\textwidth]{figures/jtd/figure_3}
  \caption[Duration of piano solo excerpts by all bandleaders in \GLS{JTD}.]{Duration of piano solo excerpts by all bandleaders in \GLS{JTD}. Bar colour indicates subset (either \GLS{JTD} or \GLS{JTD-300}).}
...
The \texttt{ZFTurbo} audio source separation model \citep{Solovyev2024} is used to separate each track in \GLS{JTD} into four sources; ``bass'', ``drums'', ``vocals'', and ``other''. \added[id=HC]{This model is an ensemble of multiple existing hybrid transformer source separation models \citep{Rouard2022_Hybrid}, with weights for each model obtained by creating synthetic datasets of audio mixtures.} In a recent community sound demixing competition, \texttt{ZFTurbo} achieved the first prize \citep{Fabbro2024}.\footnote{Note that the \texttt{SAMI-ByteDance} model \citep{Lu2023_SourceSep} achieved higher numerical scores than \texttt{ZFTurbo} in this competition. However, as this model has not yet been released publicly, it was not awarded a prize and could not be used here.}

\subsubsection{Audio Channel Selection}

Before the development of full-spectrum stereophonic panning in the 1970s, audio signals could only be panned to the left or right speaker output, or both (centre channel). For tracks in \GLS{JTD} where at least one instrument in the trio was hard-panned in this manner (39\%), source separation is applied independently to the left and right monaural signals alongside the centre output\added[id=HC]{, in order to obtain the cleanest result for each instrument \citep[see][]{Riley2023}}. The required channel is identified manually for each instrument during the metadata curation process (see Section~\ref{sec:jtd_metadata_curation}).

\subsubsection{Audio Pre-processing}\label{sec:jtd_audio_preprocessing}
...
\algnewcommand{\LineComment}[1]{\State \(\triangleright\) \texttt{#1}}
\caption[Beat tracking with iterative refinement of minimum and maximum tempi.]{Beat tracking with iterative refinement of minimum and maximum tempi.}
\label{alg:jtd_beat_track_iteration}
\begin{algorithmic}[1]
\Require{$A$ (mixture audio signal), $I$ (number of iterations)}
\Ensure{$B'$ (array of quarter-note beat timestamps)}
\LineComment{Use the default minimum and maximum tempi for the first iteration}
\State $T_{min} \gets 100$
\State $T_{max} \gets 300$
\For{$i \gets 1$ to $I$}
    \LineComment{Estimate beat positions using the network}
    \State $B \gets \texttt{DBNDownBeatTracker}(A, T_{min}, T_{max})$
    \State $N \gets \text{length}(B) - 1$
    \LineComment{Compute durations of successive beats}
    \State Initialize array $D$ of size $N$
    \For{$n \gets 1$ to $N$}
        \State{$D[n] \gets B_{n+1} - B_n$}
    \EndFor
    \LineComment{Compute quartiles and interquartile range}
    \State $D_{s} \gets \text{sorted}(D)$
    \State $Q_{1} \gets \text{quartile}(D_{s}, \ 0.25)$
    \State $Q_{3} \gets \text{quartile}(D_{s}, \ 0.75)$
    \State $R_D \gets Q_3 - Q_1$  
    \LineComment{Filter beat array based on interquartile range}
    \State Initialize array $D_{f}$ as \texttt{empty}
    \For{$d \in D$}
        \If{$d \geq Q_1 - 1.5R_D$ \textbf{and} $d \leq Q_3 + 1.5R_D$}
            \State Append $d$ to $D_{f}$
        \EndIf
    \EndFor
    \If{$D_{f}$ is \texttt{empty}}
        \State \textbf{break}
    \Else
        \LineComment{Compute new minimum and maximum tempi}
        \State $T_{min} \gets \dfrac{60}{\text{max}(D_f)}$ 
        \State $T_{max} \gets \dfrac{60}{\text{min}(D_f)}$ 
        \State \textbf{continue}
    \EndIf
\EndFor
\LineComment{Final estimation}
\State $B^{'} \gets \texttt{DBNDownBeatTracker}(A, T_{min}, T_{max})$
\State \Return{$B^{'}$}
\end{algorithmic}
\end{algorithm}

We design an algorithm that applies the \texttt{DBNDownBeatTracker} to an audio signal multiple times (Algorithm \ref{alg:jtd_beat_track_iteration}). The \texttt{DBNDownBeatTracker} takes as arguments the minimum and maximum allowed tempi for detected beats. We set this initially to 100 and 300 beats-per-minute, respectively, as in our inclusion criteria (Section \ref{sec:jtd_inclusion_criteria}). Over a number of iterations $I$, we then apply the tracker, calculate the instantaneous tempo at every beat, remove tempo values that are outliers, and recalculate the minimum and maximum allowed tempi using the new, filtered, beat timestamps. We \replaced[id=HC]{obtain a single, optimised value of $I$ for all the recordings in \GLS{JTD} using the process described in Section~\ref{sec:jtd_validation}}{optimise $I$ specifically for the recordings in \GLS{JTD} (see Section \ref{sec:jtd_validation})}. The downbeat classes estimated from the \GLS{RNN} are then combined with the time signature for the recording, in order to assign measure numbers to every timestamp detected with Algorithm \ref{alg:jtd_beat_track_iteration}. 
...
We show the values of $F$ obtained from the optimised parameter set in Table~\ref{tab:jtd_optimization_results}. The performance of the onset detection algorithm is broadly equivalent across instrument classes, with drums performing marginally better than bass and piano. In comparison with earlier results obtained from applying the same algorithm to equivalent multitracked audio datasets (``polyphonic pitched'', $\text{mean}(F) = 0.95$, ``plucked strings'', $\text{mean}(F) = 0.90$, ``solo drums'', $\text{mean}(F) = 0.93$, see Footnote~\ref{note:mirex_2018}), we obtain similar onset detection accuracy. This indicates that the source separation process is unlikely to have caused significant damage (in the form of artifacts, etc.) to the audio, to the point that onset detection algorithms trained on multitracked recordings cannot also be applied to \GLS{JTD}.
...
\begin{figure}[!ht]
  \centering
  \includesvg[width=0.75\textwidth]{figures/jtd/figure_x2}
  \caption[Onset detection results per window size.]{Onset detection results per window size. The $x$-axis shows the size of the window used to match detected and ground-truth onsets and the $y$-axis shows the average value of $F$ across all recordings in the ground truth subsample. Results are stratified by instrument (line and marker colours). Optimisation is performed separately for every combination of window and instrument.}
\label{fig:jtd_f_vs_window_size}
\end{figure}

\added[id=HC]{Finally, in Figure~\ref{fig:jtd_f_vs_window_size}, we consider the effects of window size on $F$ by optimising onset detection separately for each instrument, using window sizes $W \in \{10, 25, 50, 100, 125, 150\}$ ms. The results suggest that windows wider than 50 ms do not yield substantial improvements in mean $F$, and therefore that this value is likely optimal for demonstrating the upper performance boundary of our method. Equally, our results suggest that performance is not substantially degraded with a window half as narrow as this (i.e,. $W = 25$ ms).}\footnote{The sharp decrease in performance below this can perhaps be explained by noise in the annotated examples.}

\subsubsection{Alternative Methods}\label{sec:jtd_alternate_methods}

Next, we compare several alternative onset detection methods, using the piano audio and corresponding ground truth annotations for evaluation. These methods are: 
\begin{enumerate}
    \item{A high-resolution automatic piano transcription model \citep{Kong2021}, which generates MIDI (incorporating pitch, note and pedal onset and offset times, and velocity) from piano audio using a convolutional recurrent neural network (\GLS{CRNN}),\footnote{At the tine \GLS{JTD} was compiled, the state-of-the-art method for automatic piano transcription incorporating note onset, offset, and velocity information was \citet{Hawthorne2021}. However, the authors of this model report worse performance than the model described by \citet{Kong2021} when considering only note onset information; given that our evaluation only considers onsets, we elected to use the earlier model, instead.}}
    \item{A naive audio signal processing approach, where onsets are identified as local maxima from a normalised spectral flux envelope, using the implementation from the \texttt{librosa} (version \texttt{0.8.1}) Python package \citep{McFee2015},}
    \item{The current approach, without the audio filtering described in Section~\ref{sec:jtd_audio_preprocessing},}
    \item{The current approach, with a quantitatively ``narrower'' band-pass filter (attenuating frequencies outside the range of 110--1760 Hz i.e., $A_2$--$A_6$).}
\end{enumerate}

Note that, as method (1) generates multiple MIDI events for what would be regarded perceptually as a single onset (i.e., a chord), near-simultaneous MIDI events within a window of \replaced[id=HC]{100}{50} ms are grouped together, with the \replaced[id=HC]{mean onset time}{onset of the earliest event} taken as the onset of the whole group. The parameters exposed by methods (1--4) are optimised separately, following the approach described in Section~\ref{sec:jtd_validation_results}. The default setting of 100 frames per second is used for method (1) and only note onsets are considered, with pedal onsets discarded. No other post-processing is applied, in order to test the baseline performance of each system.
...
\textbf{\citet{Kong2021} (1)} & \added[id=HC]{0.82 (0.07)} & \added[id=HC]{0.80 (0.09)} & \added[id=HC]{0.84 (0.06)} \\
...
The results from this comparison are given in Table~\ref{tab:jtd_methods_comparison}. For detecting onsets in the \GLS{JTD} piano audio, the current approach outperforms all four alternatives. In particular, we note how ``narrowing'' the band-pass filter seems to affect the precision-recall trade-off of the CNN predictions: the narrower filter setting (4), for example, results in a lower false positive rate than the settings described in Section~\ref{sec:jtd_audio_preprocessing}, but a higher false negative rate. The current approach, with filter settings between those of methods (3--4), results in approximately equal precision and recall. All three CNN-based methods significantly outperform spectral flux.

The results obtained for method (1) are \added[id=HC]{somewhat }disappointing, especially comparing with those given by the authors. One possibility is that this model may have overfitted to the acoustic properties of the instrument(s) in its training data and be less resilient to the varied recording conditions contained in \GLS{JTD}. More robust approaches that make use of, e.g., augmented training data may be worth exploring in the future \citep[here, see][]{Edwards2024}. Another is that the model might ``repeat'' notes (producing multiple MIDI values for a single sustained pitch) or demonstrate octave errors (producing multiple MIDI notes at different octaves for a single pitch). If so, post-processing of the transcribed MIDI may help when using this data for downstream applications (see, e.g., Sections \ref{sec:rsi_handcrafted_feature_extraction}, \ref{sec:gen_preprocessing}).

Regardless, we include these MIDI annotations in \GLS{JTD} and make use of them in both Chapters \ref{chap:xai_rsi} and \ref{chap:conditioned_gen}. In both cases, we are interested solely in the performance of the piano soloist in a recording, and not how this relates to the rest of the ensemble. The total number of MIDI events is 2,174,833 (approximately $2.5$x the number of total piano onsets).

\subsubsection{Bass Annotation}\label{sec:jtd_bass_detection}

Considering the results in Table~\ref{tab:jtd_optimization_results}, the higher standard deviation for the bass onset detection results suggests that there may be a subset of onset types that give outlier results, diminishing the overall accuracy of the pipeline. While we cannot investigate every possibility, we can consider three hypotheses: 
\begin{enumerate}
    \item{Was the bass annotation affected when notes were ``raked''?}
    \item{Was annotation affected when the bass and drums played simultaneously?}
    \item{Was annotation worse on specific beats of the bar?}
\end{enumerate}
...
There is considerable variation in the duration of solos played by different pianists (Figure~\ref{fig:jtd_mean_track_duration_by_pianist}). In particular, the later a pianist's first recording in \GLS{JTD} was made (i.e., the later they began their career), the longer they tended to solo for. For the 9 (out of 27) \replaced[id=HC]{pianist-bandleaders}{pianists} in \GLS{JTD} with the longest average solo durations, the median year their first recording was made is 1967; for the 9 \replaced[id=HC]{pianist-bandleaders}{pianists} with the shortest solos on average, the median year is 1956. 

One possible explanation is that the open-ended formal designs (e.g., modal ``vamps'', ``time, no changes'') that became popular in jazz during the latter half of the twentieth century proved more suitable for extended improvisation, which does not need to follow the harmonic structure of an underlying ``song form'' \citep{Gioia2011}. Changing trends in the music business could provide another explanation; up until the mid-1950s, record companies would often fill albums with multiple shorter recordings rather than longer individual takes, so that individual tracks could be released as singles to be played on jukeboxes and radio stations \citep{Priestley1991}.
...
\bar{y} = \dfrac{\sum\limits_{i=2}^N\frac{60}{x_i - x_{i-1}}}{N-1} \ ,
\end{align}
where $x_i$ is the mean of all onsets matched to beat $i$ ($i \geq 1$) and neither $x_i$ or $x_{i-1}$ are set to missing. Across \GLS{JTD}, the observed $\text{mean}(\bar{y}) = 193.92 \ \text{BPM}$ ($SD = 46.93$, Figure~\ref{fig:jtd_mean_tempo}); this is comparable to the equivalent result calculated using the timestamps $y'$ estimated from the beat tracking algorithm ($\text{mean}(\bar{y'}) = 193.70, SD = 46.59$). The slowest performance in \GLS{JTD} has a mean tempo of 102.10 BPM (Cedar Walton, ``Ghost of a Chance'', 2010) and the fastest 299.82 BPM (John Hicks, ``Airegin'', 1987).

As a measure of tempo stability, for each track we obtain the standard deviation of the tempo normalised by the mean tempo i.e., the percentage fluctuation about the overall tempo (``tempo fluctuation'', $\delta y$). This can be written simply as
\begin{align}\label{eq:tempo_fluctuation}
\delta y = \dfrac{\sqrt{\frac{1}{N-1} \sum\limits_{i=2}^N (y_i - \bar{y})^2}}{\bar{y}} \ ,
\end{align}
where $y_i$ is the tempo value in quarter-note beats-per-minute at beat $i$ (i.e., $y_i = \dfrac{60}{x_i-x_{i-1}}$). We observe $\text{mean}(\delta y) = 5.55\%$ ($SD = 2.70$) for \GLS{JTD} recordings, suggesting that they are generally stable.

We also calculate the slope of a linear regression of instantaneous tempo against beat onset time (``tempo slope'', $\hat{S}$) using the standard least squares formula
\begin{align}\label{eq:tempo_slope}
\hat{S} = \frac{\sum\limits_{i=2}^N (x_i - \bar{x}) (y_i - \bar{y})}{\sum\limits_{i=1}^N (x_i - \bar{x})^2} \ ,
\end{align}
...
There is no correlation between signed tempo slope and unsigned tempo fluctuation, $r_{\hat{S}, \ \delta y}(1292) = -0.06, p = 0.06$; however, there is a positive correlation between the unsigned slope (i.e., any global change in tempo, regardless of direction) and fluctuation, $r_{|\hat{S}|, \ \delta y}(1292) = 0.37, p < .001$. There are positive correlations between both mean tempo and signed tempo slope, $r_{\bar{y}, \ \hat{S}}(1292) = 0.25, p < .001$, mean tempo and unsigned slope, $r_{\bar{y}, \ |\hat{S}|}(1292) = 0.60, p < .001$, and mean tempo and fluctuation, $r_{\bar{y}, \ \delta y}(1292) = 0.24, p < .001$. This suggests that performances which have a faster pace tend to involve a greater magnitude of tempo change than those which are slower, and that these performances are also less stable overall.

\subsection{Swing}\label{sec:jtd_swing}

In jazz, swing refers to the division of the pulse into alternating long and short intervals. Expressed in Western notation, the long interval is typically written as a quarter note triplet, and the short as an eighth note triplet. Empirically, swing can be measured by taking the ratio of these long and short durations --- the swing or ``beat-upbeat ratio'' (\textit{\GLS{BUR}}), commonly expressed in binary logarithmic form in the literature. This can be written simply as
\begin{align}\label{eq:jtd_bur}
log_2(\mathit{BUR})=log_2\biggl(\frac{t_{a,b}-t_{a}}{t_{b}-t_{a,b}}\biggr) \ ,
\end{align}
where $t_a$ is the onset matched with beat $a$, $t_b$ is the onset matched with beat $b$ (i.e., at beat $a + 1$), and $t_{a,b}$ is the single onset between beats $a$ and $b$. For notated ``swung'' eighths, $log_2(\mathit{BUR}) = 1.0$ ($\mathit{BUR} = 2:1$); vice versa, for ``straight'' eighths (i.e., the equal-equal subdivision of the beat), $log_2(\mathit{BUR}) = 0.0$ ($\mathit{BUR} = 1:1$).

We search our database for all discrete groupings of three onsets, where the first and last mark the quarter note pulse. The total number of such groupings is 417,278. Following the analysis of \GLS{WJD} by \citet{Corcoran2021}, we classify ratios above 4:1 ($log_2(\mathit{BUR}) = 2$) and below 1:4 ($log_2(\mathit{BUR}) = -2$) as outliers, which results in the exclusion of 2.10\% of these groupings. The final number of beat-upbeat ratios in the dataset is 408,506 (piano: 151,640, bass: 47,115, drums: 209,751).
...
\begin{sidewaysfigure}[ph!]
  ...
  \caption[Distribution of $log_2(\mathit{BUR})$s between instruments across \GLS{JTD}]{Distribution of $log_2(\mathit{BUR})$s between instruments across \GLS{JTD}, normalised such that the height of the largest bar in each panel is 1. Dotted vertical lines show peaks of the density estimates; straight lines correspond with the musical notation given along the top of the panel.}
\label{fig:jtd_bur_distribution_by_instrument}
\end{sidewaysfigure}

For the piano, we find one peak in the density estimate, corresponding with $log_2(\mathit{BUR})$ of 0.37 ($\mathit{BUR} = 1.44:1$, 95\% CI: $[0.15, 0.43]$). For the bass, we find two peaks, corresponding with $log_2(\mathit{BURs})$ of 0.06 ($1.06:1$, $[0.03, 0.13]$) and 1.23 ($3.41:1$, $[1.15, 1.27]$). For the drums, we also find two peaks, at $log_2(\mathit{BUR}) = -0.91$ ($0.40:1$, $[-1.15, -0.70]$) and 1.18 ($3.25:1$, $[1.13, 1.23]$). These density estimates and the corresponding peaks are shown in Figure~\ref{fig:jtd_bur_distribution_by_instrument}.

This analysis suggests that (1) all instrument classes primarily target long-short divisions of the beat, with the percentage of total $log_2(\mathit{BURs}) > 0.0 = 88.6\%$ (piano: 81.9\%, bass: 89.1\%, drums: 93.4\%); (2) pianist beat-upbeat ratios are closer to notated ``straight'' than ``swung'' eighths, judging by the peak of their density estimate; (3) in contrast, the larger of the two peaks in the bass and drum\deleted[id=HC]{s} density estimates suggest that both instruments tend towards higher beat-upbeat ratios than would be implied by notated swing eighths alone. 
...
Next, we consider the relationship between swing and the tempo of a performance. We fit a linear mixed effects model using the implementation in the \texttt{statsmodels} (version \texttt{0.13.1}) Python library \citep{Seabold2010}. We predict a performer's mean $log_2(\mathit{BUR})$ using the mean tempo of the recording (standardised through $z$-transformation), their instrument, and the interaction between tempo and instrument as fixed effects (with piano as the reference category). Bandleader is used as a random effect (slopes and intercepts). An individual musician's performance is excluded if 15 ratios could not be obtained, which results in 471 individual performances out of 3,865 being excluded (piano: 30, bass: 401, drums: 40).

\begin{figure}[!ht]
  \centering
  \includesvg[width=0.75\textwidth]{figures/jtd/figure_9}
  \caption[Mean $log_2(\mathit{BUR})$ and tempo for an individual recording and instrument in \GLS{JTD}.]{Mean $log_2(\mathit{BUR})$ and tempo for an individual recording and instrument in \GLS{JTD}. Solid lines show predictions (without random effects), shaded areas show 95\% confidence intervals (obtained via bootstrapping over data from different pianists, $N = 10,000$)}
\label{fig:jtd_bur_vs_tempo}
\end{figure}

An increase in mean tempo is a significant predictor of a decrease in mean beat-upbeat ratio for a recording (Figure~\ref{fig:jtd_bur_vs_tempo}), with a one $SD$ change in BPM associated with a decrease of -0.14 mean $log_2(\mathit{BUR})$ ($p < .001$, 95\% CI: $[-0.16, -0.12]$). This suggests that it becomes harder for musicians to articulate long-short subdivisions of the quarter note as its duration decreases. This ``straightening'' effect has also been observed in analyses of both \GLS{WJD} \citep{Corcoran2021} and FiloSax \citep{Foster2021}.
...
We fit the model described in Section~\ref{sec:jtd_effect_of_tempo_on_swing} with an additional fixed effect of recording year (standardised via $z$-transformation) and the interaction between year and tempo. Increases in recording year have no significant main effect on mean $log_2(\mathit{BUR})$ ($\beta = 0.01, p = 0.44$, 95\% CI: $[-0.02, 0.05]$), and there is no significant interaction between year and tempo ($\beta < 0.01, p = 0.79$, 95\% CI: $[-0.01, 0.01]$). Including the year of recording in the model also results in minimal change to the amount of variance it explains (conditional $R_2 = 62.8\%$, marginal $R_2 = 59.3\%$).

This suggests that the year a track was recorded has minimal impact on the levels of swing displayed by the musicians. These results should not be taken as indicative of global trends in swing timing, however, as the recordings in \GLS{JTD} are mostly representative of the ``straight ahead'' jazz style, as practised throughout the last century. Prior analysis of \GLS{WJD} has demonstrated tangible differences in swing between jazz styles \citep{Corcoran2021}.

\subsubsection{Comparison with Existing Datasets}\label{sec:jtd_dataset_comparison}

Finally, we want to compare the levels of ``swing'' demonstrated in \GLS{JTD} with existing open-source datasets of annotated jazz performances. We select \GLS{WJD} for comparison to \GLS{JTD} piano recordings and FiloBass for comparison to \GLS{JTD} bass recordings. 

\begin{sloppypar}
We scrape all tracks from \replaced[id=HC]{\GLS{WJD}}{both databases} that meet the tempo and style inclusion criteria for \GLS{JTD}; \added[id=HC]{this means that we include tracks where \texttt{instrument == "piano"}, \texttt{rhythmfeel == "SWING"}, and $100 \leq \texttt{avgtempo} \leq 300$, using the metadata provided in the \texttt{solo\_info} table. This leaves 6 out of 456 recordings. For FiloBass, as all tracks have a 4/4 time signature, a tempo between 111 and 226 BPM, and a consistent ``swing'' feel \citep{Foster2021}, we can compare the entirety of this database (48 tracks) with \GLS{JTD} directly.} No tracks are duplicated across the three datasets; however, some of the same performers do appear, including bassists Ron Carter and Ray Drummond in both FiloBass and \GLS{JTD}.
\end{sloppypar}

\begin{sidewaysfigure}[ph!]
  \includesvg[width=1.0\textwidth]{figures/jtd/figure_x1_alt}
  \caption[Distribution of $log_2(\mathit{BUR})$s for recordings in \GLS{WJD} and FiloBass]{Distribution of $log_2(\mathit{BUR})$s for recordings in \GLS{WJD} and FiloBass, following the format in Figure~\ref{fig:jtd_bur_distribution_by_instrument}.}
\label{fig:jtd_bur_distribution_by_dataset}
\end{sidewaysfigure}

We then match onset annotations from both \GLS{WJD} and FiloBass with their nearest annotated quarter-note beats and extract \textit{\GLS{BUR}s}. Only downbeat annotations are provided for FiloBass, however, so we annotate the remaining beats between successive downbeat timestamps through linear interpolation\replaced[id=HC]{, assuming a 4/4 time signature}{according to the time signature of the recording}. As outlined in Section~\ref{sec:jtd_swing}, we discard \textit{\GLS{BUR}s} outside the range of $-2 < log_2(\mathit{BUR}) < 2$. We extract 707 \textit{\GLS{BUR}s} for \GLS{WJD} and \replaced[id=HC]{3,414}{3,218} for FiloBass. We show normalised kernel density and peak estimates for the $log_2(\mathit{BUR})$ distributions in both datasets in Figure~\ref{fig:jtd_bur_distribution_by_dataset}, following the process outlined in Section~\ref{sec:jtd_differences_in_swing_between_instruments}.

With regard to the piano, the distribution of $log_2(\mathit{BUR})$s in \GLS{WJD} is broadly equivalent to \GLS{JTD}, with one peak found at $log_2(\mathit{BUR}) = 0.05$ ($1.05:1$). This is considerably lower than the equivalent peak in \GLS{JTD} ($log_2(\mathit{BUR}) = 0.37, 1.44:1$). Inspecting this, we find that the mean tempo of the six piano recordings in \GLS{WJD} (275.63 BPM) is much higher than across \GLS{JTD} (193.92). As we have already shown in Figure~\ref{fig:jtd_bur_vs_tempo}, higher tempi lead to demonstrably ``straighter'' \textit{\GLS{BUR}} values.

With regard to the bass, we find two peaks in the distribution of \textit{\GLS{BUR}s} obtained from FiloBass, at \replaced[id=HC]{$log_2(\mathit{BUR}) = -0.23$ ($0.85:1$)}{$log_2(\mathit{BUR}) = -0.22$ ($0.80:1$)} and \replaced[id=HC]{$log_2(\mathit{BUR}) = 1.22$ ($2.33:1$)}{$log_2(\mathit{BUR}) = 1.23$ ($3.41:1$)}. These are broadly similar to the two peaks found in \GLS{JTD} (see Figure~\ref{fig:jtd_bur_distribution_by_instrument}). However, we note that the proportion of ``straight'' bass \textit{\GLS{BUR}s} (where $log_2(\mathit{BUR})\approx0$) is significantly higher in FiloBass than in \GLS{JTD}. One possibility is that the recordings in FiloBass are ``play-along'' backing tracks (as opposed to commercial recordings), meaning that there was no soloist actually improvising during the recording session. Another possibility is simply increased variance in the data, due to the smaller number of recordings in FiloBass versus \GLS{JTD}.

\subsection{Synchronisation}\label{sec:jtd_synchrony}

\replaced[id=HC]{Synchrony can be quantified by considering the precise alignment of}{Synchrony can be defined as the difference between} onsets that mark the same musical event, such as a quarter note beat. While it can be expressed in absolute or ``raw'' units (milliseconds, frames), we express synchrony in relative terms instead. \added[id=HC]{In \citet{Rasch1988}, absolute onset times are first converted to relative onset times by expressing each relative to the mean onset of all performers at that moment in the performance. The time differences between relative onsets can then be used to measure asynchrony. We use a different method, expressing onset time differences between performers relative to the duration of a single quarter note at the mean tempo of a performance. This allows us to compare across performances made at different tempi in a musically intuitive fashion:} for example, a value of $+25\%$ implies that one musician plays a sixteenth note later than another.
...
\item[--]{The instrument labels and audio files in \GLS{JTD} were included as part of the dataset used to train Aria-CL, a solo piano classifier.\footnote{\url{https://github.com/loubbrad/aria-cl}} This model, in turn, has been employed to create a new dataset of automatically transcribed solo piano recordings in MIDI format, spanning many musical genres \citep{Bradshaw2025}.}
\item[--]{In Chapter \ref{chap:conditioned_gen}, we link the recordings in \GLS{JTD} with high-quality subgenre annotations using an online music metadata service. As we go on to describe, we then use these annotations to condition a generative model to produce music in the style of these subgenres, as well as particular performers in \GLS{JTD} including Bill Evans. We make both our model and these annotations available as an extension of \GLS{JTD}.}
\end{itemize}
...
In the following chapters, we explore the ways in which \GLS{JTD} can be deployed to model the factors contributing towards improvisation style (Chapter~\ref{chap:xai_rsi}) and the use of rhythm in jazz (Chapter~\ref{chap:rhythm_rsos}). We also use \GLS{JTD} as part of the training data for a style-conditioned generative music model (Chapter \ref{chap:conditioned_gen}).