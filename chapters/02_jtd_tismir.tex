...
\For{$i \gets 1$ to $I$}
    ...
    \State $Q_{1} \gets D_{s}[\lfloor 0.25 \times N \rfloor]$
    \State $Q_{3} \gets D_{s}[\lfloor 0.75 \times N \rfloor]$
    ...
    \For{$d \in D$}
        \If{$d \geq Q_1 - 1.5 \times IQR_D$ \textbf{and} $d \leq Q_3 + 1.5 \times IQR_D$}
...
We design an algorithm that applies the \texttt{DBNDownBeatTracker} to an audio signal multiple times while iteratively refining the range of tempi used for the tracked quarter-note beats (Algorithm \ref{alg:jtd_beat_track_iteration}). We optimise the total number of iterations this algorithm runs for (input parameter $I$) specifically for the recordings in \GLS{JTD} (see Section~\ref{sec:jtd_validation}). The downbeat classes estimated from the \GLS{RNN} are then combined with the time signature for the recording, in order to assign measure numbers to every timestamp detected with Algorithm \ref{alg:jtd_beat_track_iteration}. 
