...
We then match onset annotations from both \GLS{WJD} and FiloBass with their nearest annotated quarter-note beats and extract \textit{\GLS{BUR}s}. Only downbeat annotations are provided for FiloBass, however, so we annotate the remaining beats between successive downbeat timestamps through linear interpolation \replaced[id=HC]{, assuming a 4/4 time signature}{according to the time signature of the recording}. As outlined in Section~\ref{sec:jtd_swing}, we discard \textit{\GLS{BUR}s} outside the range of $-2 < log_2(\mathit{BUR}) < 2$. We extract 707 \textit{\GLS{BUR}s} for \GLS{WJD} and \replaced[id=HC]{}{3,218} for FiloBass. We show normalised kernel density and peak estimates for the $log_2(\mathit{BUR})$ distributions in both datasets in Figure~\ref{fig:jtd_bur_distribution_by_dataset}, following the process outlined in Section~\ref{sec:jtd_differences_in_swing_between_instruments}.
